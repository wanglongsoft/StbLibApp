//
// Created by 24909 on 2020/7/9.
//

#include "FrameBufferTexture.h"

FrameBufferTexture::FrameBufferTexture(GlobalContexts *global_context) {
    gl_program = -1;
    gl_uMatrix = -1;
    gl_position = -1;
    gl_textCoord = -1;
...
GLuint FrameBufferTexture::LoadShader(GLenum type, const char *shaderSrc) {
...
GLuint FrameBufferTexture::LoadProgram(const char *vShaderStr, const char *fShaderStr) {
    LOGD("LoadProgram in");
    GLuint vertexShader;
...
int FrameBufferTexture::createProgram() {
...
void FrameBufferTexture::render(unsigned char *data) {
    LOGD("FrameBufferTexture::render in");
    this->rgba_data = data;
    ...
    LOGD("FrameBufferTexture::render out");
}

void FrameBufferTexture::setImageSize(int width, int height) {
    this->gl_image_width = width;
    this->gl_image_height = height;
}

void FrameBufferTexture::setWindowSize(int width, int height) {
    this->gl_window_width = width;
    this->gl_window_height = height;
}

void FrameBufferTexture::initDefMatrix() {
...
void FrameBufferTexture::orthoM(float *m, int mOffset, float left, float right, float bottom, float top,
...
float FrameBufferTexture::adjustRatio(float ratio, float worldRatio) {
...
void FrameBufferTexture::readPixelData() {
    LOGD("readPixelData in");

//    read_image_width = gl_window_width;
//    read_image_height = gl_window_height;

    read_image_width = gl_image_width;
    read_image_height = gl_image_height;
...
void FrameBufferTexture::savePixelData() {
...
void FrameBufferTexture::initShaderAttributes(GLint program, bool is_frame_program) {
    //获取顶点着色器和片段着色器句柄句柄
...
void FrameBufferTexture::initCommonAttributes() {
    //创建若干个纹理对象，并且得到纹理ID
    ...
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT32F,
            context->gl_image_width, context->gl_image_height);

    glBindFramebuffer(GL_FRAMEBUFFER, frameBuffer);

    glBindTexture(GL_TEXTURE_2D, gl_texture_id[0]);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, gl_texture_id[0], 0);
    glBindTexture(GL_TEXTURE_2D, GL_NONE);
