...
VertexBufferDisplay::VertexBufferDisplay(GlobalContexts *global_context) {
    gl_program = -1;
    gl_uMatrix = -1;
    gl_position = -1;
    gl_textCoord = -1;
...
VertexBufferDisplay::~VertexBufferDisplay() {
...
GLuint VertexBufferDisplay::LoadShader(GLenum type, const char *shaderSrc) {
...
GLuint VertexBufferDisplay::LoadProgram(const char *vShaderStr, const char *fShaderStr) {
    LOGD("LoadProgram in");
    GLuint vertexShader;
...
int VertexBufferDisplay::createProgram() {
...
void VertexBufferDisplay::render(unsigned char *data) {
    this->rgba_data = data;
...
void VertexBufferDisplay::setImageSize(int width, int height) {
    this->gl_image_width = width;
    this->gl_image_height = height;
}

void VertexBufferDisplay::setWindowSize(int width, int height) {
    this->gl_window_width = width;
    this->gl_window_height = height;
}

void VertexBufferDisplay::initDefMatrix() {
...
void VertexBufferDisplay::orthoM(float *m, int mOffset, float left, float right, float bottom, float top,
...
float VertexBufferDisplay::adjustRatio(float ratio, float worldRatio) {
