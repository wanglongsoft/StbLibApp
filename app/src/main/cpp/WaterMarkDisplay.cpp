...
WaterMarkDisplay::WaterMarkDisplay(GlobalContexts *global_context) {
    gl_program = -1;
    gl_uMatrix = -1;
    gl_position = -1;
    gl_textCoord = -1;
...
WaterMarkDisplay::~WaterMarkDisplay() {
    ...
    glDeleteTextures(1, &gl_water_id);

    glDeleteProgram(gl_program);
    glDeleteBuffers ( 1, &gl_coord_id);
}

GLuint WaterMarkDisplay::LoadShader(GLenum type, const char *shaderSrc) {
...
GLuint WaterMarkDisplay::LoadProgram(const char *vShaderStr, const char *fShaderStr) {
    LOGD("LoadProgram in");
    GLuint vertexShader;
    ...
    u_texture = glGetUniformLocation(gl_program, "uTexture");
    water_mark = glGetUniformLocation(gl_program, "waterMark");
    LOGD("glGetUniformLocation u_texture: %d", u_texture);
    LOGD("glGetUniformLocation water_mark: %d", water_mark);
    ...
    //将纹理目标和纹理绑定后，对纹理目标所进行的操作都反映到对纹理上
    glGenTextures(1, &gl_water_id);
    glBindTexture(GL_TEXTURE_2D, gl_water_id);

    glTexImage2D(GL_TEXTURE_2D,
                 0,//细节基本 默认0
                 GL_RGBA,
                 context->gl_water_image_width,//加载的纹理宽度
                 context->gl_water_image_height,//加载的纹理高度
                 0,//纹理边框
                 GL_RGBA,//数据的像素格式, RGB数据格式
                 GL_UNSIGNED_BYTE,//像素点存储的数据类型
                 NULL //纹理的数据（先不传）
...
int WaterMarkDisplay::createProgram() {
    ...
    fragment_shader_graphical = ShaderUtils::openAssetsFile(this->context->assetManager, "fragment_filter_water_mark_display.glsl");
...
void WaterMarkDisplay::render(unsigned char *src_data, unsigned char *water_data) {
    this->rgba_data = src_data;
    ...
    glTexSubImage2D(GL_TEXTURE_2D,
                    ...
                    0, //相对原来的纹理的offset
                    0,//相对原来的纹理的offset
                    context->gl_water_image_width,
                    context->gl_water_image_height,//加载的纹理宽度、高度。最好为2的次幂
                    GL_RGBA,
                    GL_UNSIGNED_BYTE,
                    this->water_data);
    glUniform1i(water_mark, 1);
...
void WaterMarkDisplay::setImageSize(int width, int height) {
    this->gl_image_width = width;
    this->gl_image_height = height;
}

void WaterMarkDisplay::setWindowSize(int width, int height) {
    this->gl_window_width = width;
    this->gl_window_height = height;
}

void WaterMarkDisplay::initDefMatrix() {
...
void WaterMarkDisplay::orthoM(float *m, int mOffset, float left, float right, float bottom, float top,
...
float WaterMarkDisplay::adjustRatio(float ratio, float worldRatio) {
