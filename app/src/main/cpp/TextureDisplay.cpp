...
TextureDisplay::TextureDisplay(GlobalContexts *global_context) {
    gl_program = -1;
    gl_uMatrix = -1;
    gl_position = -1;
    gl_textCoord = -1;
...
GLuint TextureDisplay::LoadShader(GLenum type, const char *shaderSrc) {
...
GLuint TextureDisplay::LoadProgram(const char *vShaderStr, const char *fShaderStr) {
    LOGD("LoadProgram in");
    GLuint vertexShader;
...
int TextureDisplay::createProgram() {
...
//    fragment_shader_graphical = ShaderUtils::openAssetsFile(this->context->assetManager, "fragment_split_screen.glsl");// 9分屏测试
...
void TextureDisplay::render(unsigned char *data) {
    this->rgba_data = data;
...
void TextureDisplay::setImageSize(int width, int height) {
    this->gl_image_width = width;
    this->gl_image_height = height;
}

void TextureDisplay::setWindowSize(int width, int height) {
    this->gl_window_width = width;
    this->gl_window_height = height;
}

void TextureDisplay::initDefMatrix() {
//    if((gl_image_width < gl_window_width) && (gl_image_height < gl_window_height)) {// 图片宽高小于视图宽高，原图显示
//        float widthRatio = (float) gl_window_width / gl_image_width;
//        float heightRatio = (float) gl_window_height / gl_image_height;
//        orthoM(
//                matrix_scale, 0,
...
void TextureDisplay::orthoM(float *m, int mOffset, float left, float right, float bottom, float top,
...
float TextureDisplay::adjustRatio(float ratio, float worldRatio) {
