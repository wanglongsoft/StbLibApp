...
MapBuffersDisplay::MapBuffersDisplay(GlobalContexts *global_context) {
    gl_program = -1;
    gl_uMatrix = -1;
    gl_position = -1;
    gl_textCoord = -1;
...
MapBuffersDisplay::~MapBuffersDisplay() {
...
GLuint MapBuffersDisplay::LoadShader(GLenum type, const char *shaderSrc) {
...
GLuint MapBuffersDisplay::LoadProgram(const char *vShaderStr, const char *fShaderStr) {
    LOGD("LoadProgram in");
    GLuint vertexShader;
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * (3 + 2) * 4, NULL, GL_STATIC_DRAW);

    gl_coord_mapped_buf = ( GLfloat * ) glMapBufferRange(
            GL_ARRAY_BUFFER,
            0,
            sizeof(GLfloat) * (3 + 2) * 4,
            GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT);
    if(NULL == gl_coord_mapped_buf) {
        LOGD("glMapBufferRange fail, return");
        return -1;
    }

    // Copy the data into the mapped buffer
    if(!isNeedRotation) {
        memcpy(gl_coord_mapped_buf, texture_coords, sizeof(GLfloat) * (3 + 2) * 4);
    } else {
        memcpy(gl_coord_mapped_buf, texture_coords_90, sizeof(GLfloat) * (3 + 2) * 4);
    }

    // Unmap the buffer
    if (glUnmapBuffer(GL_ARRAY_BUFFER) == GL_FALSE )
    {
        LOGD("Error unmapping array buffer object.");
        return -1;
    }
    //MapBuffers
...
int MapBuffersDisplay::createProgram() {
...
void MapBuffersDisplay::render(unsigned char *data) {
    this->rgba_data = data;
...
void MapBuffersDisplay::setImageSize(int width, int height) {
    this->gl_image_width = width;
    this->gl_image_height = height;
}

void MapBuffersDisplay::setWindowSize(int width, int height) {
    this->gl_window_width = width;
    this->gl_window_height = height;
}

void MapBuffersDisplay::initDefMatrix() {
...
void MapBuffersDisplay::orthoM(float *m, int mOffset, float left, float right, float bottom, float top,
...
float MapBuffersDisplay::adjustRatio(float ratio, float worldRatio) {
