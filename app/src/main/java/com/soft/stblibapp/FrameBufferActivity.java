...
public class FrameBufferActivity extends BaseActivity {
    ...
    //使用 FBO 可以让渲染操作不用再渲染到屏幕上，而是渲染到离屏 Buffer 中，然后可以使用 glReadPixels
    // 将渲染后的图像数据读出来，从而实现在后台利用 GPU 完成对图像的处理。
    public static final String TAG = "FrameBufferActivity";
    private static final int IMAGE_FILE_SELECT_CODE = 100;

    private SurfaceView m_surface_view;
    private Button mSelectImage;
    private Button mDisplayImage;
    private Button mImageData;
        ...
        mDisplayImage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(null != mImagePath && !mImagePath.isEmpty()) {
                    FunctionControl.getInstance().openglFrameBufferDisplay(mImagePath);
                }
            }
        });

        mImageData = findViewById(R.id.get_image_data);
        mImageData.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Log.d(TAG, "onClick: ");
                FunctionControl.getInstance().savePixelData();
            }
        });
    }
